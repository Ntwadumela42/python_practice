...
def stat(strg):
    ...
    for x in strg.split(','):
      x = x.split('|')
      try:
          total_times = total_times + [total_time(x)]
      except ValueError:
          pass
    ...
    try:
        range = format_seconds_to_hhmmss(range)
        average = format_seconds_to_hhmmss(average)
        median = format_seconds_to_hhmmss(median)
    except UnboundLocalError:
        pass
    try:
        return "Range: %s Average: %s Median: %s" % (range, average, median)
    except UnboundLocalError:
        pass
